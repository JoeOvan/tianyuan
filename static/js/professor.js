let projectTemplate = {};
projectTemplate.tabbarList = '<a href="javascript:0;" data-catogoryid="${id}" data-detailid="${detailId}" class="swiper-slide tab-item">${title}</a>';
projectTemplate.contentList = '<div class="swiper-slide swiper-no-swiping col-content-item" id="companyList_${id}"></div>';
projectTemplate.updateHtml = "<img src='../../assets/images/update.png' alt='' style='margin: 30px 0;'>";
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (let index in data.data) {
                ...
                let itemId = item.id;
                let tabbarItem = projectTemplate.tabbarList.replace('${title}', itemTitle).replace("${id}", item.id).replace('${detailId}',detailId);
                let contentItem = projectTemplate.contentList.replace('${id}',itemId);
...
function getContent(id,index) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            // let title = projectTemplate.title.replace('${title}',itemTitle).replace('${createTime}', date);
